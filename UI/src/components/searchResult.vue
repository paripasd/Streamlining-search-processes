    ...
    <div v-for="d in store.getFilteredResult" :key="d.id"
      ...
      <div class="pb-8">
        <div class="flex flex-row">
          <div class="flex flex-row items-center" v-for="p in d.path">
            ...
            <svg class="h-5 w-5 flex-shrink-0 mx-1 text-gray-300" fill="currentColor" viewBox="0 0 20 20"
              aria-hidden="true">
        ...
        <div class="flex flex-row items-center space-x-1">
          ...
          <p v-for="tag in tagList.filter(item => d.tags.includes(item.label))" class="tag-text" :class="tag.color"
            :title="tag.label">{{ tag.label }}</p>
        </div>
        <div class="flex items-center justify-between bg-stone-100 rounded-lg  px-2 py-2 w-full relative">
          <p v-if="d.snippet.answer == null" id="answer-field" class="w-full text-sm text-gray-900 flex-1 pr-10">{{
            d.answer }}</p>
          <p v-else v-html="d.snippet.answer" class="w-full text-sm text-gray-900 flex-1"></p>
          <button class="transform hover:scale-110 transition duration-200  right-0"
      ...
      <div class="flex min-h-[0px] max-h-min w-full">
        ...
        <div class="w-full justify-end items-center flex flex-row">
          ...
          <p class="ml-4 mr-8 text-clip text-sm text-gray-500">{{ new
            Date(d.modificationDate).toLocaleString('en-GB').replace('T', ' ').replace('Z', '') }}</p>
        </div>
      </div>
    </div>
  </div>
